...
def build_ra_with_sbert_config(dr_method: str = "umap") -> RetrievalAugmentationConfig:
    """
    Build RA config that mirrors the repo (SBERT embeddings, GPT-3.5 summaries,
    cluster builder) but lets you choose the DR method: 'umap', 'pacmap', or 'trimap'.
    """
    ...
    tb_cfg = ClusterTreeConfig(
        ...
        clustering_params={
            "threshold": 0.1,
            "dr_method": dr_method,   # "umap", "pacmap", or "trimap"
        },
    )
...
# Runners
def run_narrativeqa(path: Path, with_raptor: bool, tree_dir: Path, embed_cache_dir: Path, seed: int, dr_method: str) -> Dict:
    data = load_jsonl(path)
    qa   = UnifiedQAModel(UQA_MODEL_NAME)
    sbert = SBertEmbeddingModel("sentence-transformers/multi-qa-mpnet-base-cos-v1")
    cfg = build_ra_with_sbert_config(dr_method=dr_method)
...
def run_quality(path: Path, with_raptor: bool, tree_dir: Path, embed_cache_dir: Path, seed: int, dr_method: str) -> Dict:
    data = load_jsonl(path)
    qa   = UnifiedQAModel(UQA_MODEL_NAME)
    sbert = SBertEmbeddingModel("sentence-transformers/multi-qa-mpnet-base-cos-v1")
    cfg = build_ra_with_sbert_config(dr_method=dr_method)
...
def run_qasper(path: Path, with_raptor: bool, tree_dir: Path, embed_cache_dir: Path, seed: int, dr_method: str) -> Dict:
    data = load_jsonl(path)
    qa   = UnifiedQAModel(UQA_MODEL_NAME)
    sbert = SBertEmbeddingModel("sentence-transformers/multi-qa-mpnet-base-cos-v1")
    cfg = build_ra_with_sbert_config(dr_method=dr_method)
...
def main():
    ...
    ap.add_argument(
        "--dr-method",
        choices=["umap", "pacmap", "trimap"],
        default="umap",
        help="Dimensionality reduction method used inside RAPTOR clustering.",
    )
    ...
    for seed in args.seeds:
        ...
        tree_dir = Path(args.tree_dir) / args.dataset / f"{seed_tag}_{args.dr_method}"
        embed_cache_dir = Path(args.baseline_embeds) / seed_tag

        t0 = time.time()
        if args.dataset == "narrativeqa":
            metrics = run_narrativeqa(
                path, args.with_raptor, tree_dir, embed_cache_dir, seed, args.dr_method
            )
